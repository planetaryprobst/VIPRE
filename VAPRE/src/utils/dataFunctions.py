...
#--------------------------------------------------------------------------
#
#   Collection of functions that handle data.
#
#--------------------------------------------------------------------------
#
#   Use:
...
def createTrajID(body,data):
    ...
    for entry in data[1:]:
        ...
        if ID not in dct.keys():
            ...
            dct[ID].append((np.array(entry[2:5]) - np.array(entry[8:11])).tolist())
...
def createVinfID(dct):
    ...
    dct['header'].append('vID')
    
    return vInf

#############################

def readDataFile(vID,body,hEntry):
    
    hEntry = hEntry2str(hEntry)
    
    filepath = 'src/data/'+body+'/' + hEntry + '/' + body + hEntry + '-vID' + str(vID) + '.txt'
    token = open(filepath,'r')
    linestoken = token.readlines()
    
    data = {}
    header = linestoken[0].split()
    
    for item in header:
        resulttoken = []
        
        for x in linestoken:
            resulttoken.append(x.split()[header.index(item)])
        
        data[item] = [float(x) for x in resulttoken[1:]]
    
    token.close()
    
    data[header[1]] = [int(x) for x in data[header[1]]]
    data[header[2]] = [bool(x) for x in data[header[2]]]
    data[header[9]] = [bool(x) for x in data[header[9]]]
